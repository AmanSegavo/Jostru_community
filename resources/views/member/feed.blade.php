            ...
            <div class="card glass mb-4 p-0 overflow-hidden" style="border-radius: 24px;">
                <div class="p-4">
                    ...
                    <div class="mb-2">
                        ...
                        @if(trim($tag))<span style="display:inline-block;padding:3px 12px;background:rgba(34,197,94,.1);color:#22c55e;border-radius:50px;font-size:.78rem;font-weight:600;margin:2px;">{{ trim($tag) }}</span>@endif
                        @endforeach
                    </div>
                    @endif

                    @if($post->link_url)
                    <div style="margin-top:8px;padding:10px 14px;background:rgba(99,102,241,.07);border-left:3px solid #6366f1;border-radius:0 10px 10px 0;">
                        <span style="color:#6366f1;">🔗</span>
                        <a href="{{ $post->link_url }}" target="_blank" rel="noopener" style="color:#6366f1;font-size:.88rem;font-weight:600;text-decoration:none;">{{ Str::limit($post->link_url, 60) }}</a>
                    </div>
                    @endif
