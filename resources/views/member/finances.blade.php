        ...
        <div class="card glass p-0 overflow-hidden mb-4" style="border-radius:20px;">
            ...
            <div class="card-header bg-transparent border-bottom p-4 data-no-export">
                ...
                <div class="filter-wrapper">
                    <form action="{{ route('member.finances') }}" method="GET" class="row g-2 align-items-center" id="filterForm">
                        ...
                        <div class="col-md-auto d-flex align-items-end mt-4">
                            @if(request('start_date') || request('end_date') || request('division_id') || request('search'))
                                <a href="{{ route('member.finances') }}" class="btn btn-sm btn-outline-secondary w-100">Reset Filter</a>
                            @endif
                        </div>
                    </form>
                </div>
                        ...
                        <tr>
                            ...
                            <td class="px-4 py-3 text-end data-no-export">
                                ...
                                @elseif($finance->proof_path)
                                    <a href="{{ asset('public/storage/' . $finance->proof_path) }}" target="_blank" class="btn btn-sm btn-outline-info rounded-pill py-0 px-2 mb-1" style="font-size:0.75rem;">Bukti</a>
                                @endif
                                
                                <form action="{{ route('member.finances.destroy', $finance->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Hapus jurnal ini? Saldo akan terpengaruh.')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn btn-sm btn-outline-danger" style="border-radius:6px; padding:0.15rem 0.4rem;" title="Hapus Jurnal">
                    ...
                    <table class="table table-hover mb-0 align-middle table-accounting">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="px-4 py-3 text-end data-no-export">
                                    @if($debt->status != 'LUNAS')
                                        <button type="button" class="btn btn-sm btn-success rounded-pill py-0 px-2 mb-1" style="font-size:0.75rem;" data-bs-toggle="modal" data-bs-target="#payDebtModal-{{ $debt->id }}">Bayar</button>
                                    @endif
                                    <form action="/member/finances/debts/{{ $debt->id }}" method="POST" class="d-inline" onsubmit="return confirm('Hapus data ini secara permanen?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-sm btn-outline-danger" style="border-radius:6px; padding:0.15rem 0.4rem;" title="Hapus">
                                    ...
                                    <div class="modal fade" id="payDebtModal-{{ $debt->id }}" tabindex="-1">
                                        <div class="modal-dialog modal-dialog-centered text-start">
                                            <form action="/member/finances/debts/{{ $debt->id }}/pay" method="POST" class="w-100">
                                                @csrf
...
<div class="modal fade" id="financeModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <form action="{{ route('member.finances.store') }}" method="POST" enctype="multipart/form-data" class="w-100">
            @csrf
            <div class="modal-content glass" style="border-radius:24px;">
                <div class="modal-header border-0 pb-0">
...
<div class="modal fade" id="budgetModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <form action="{{ route('member.budgets.store') }}" method="POST" class="w-100">
            @csrf
            <div class="modal-content glass" style="border-radius:24px;">
                <div class="modal-header border-0 pb-0">
...
<div class="modal fade" id="debtModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <form action="/member/finances/debts" method="POST" class="w-100">
            @csrf
            <div class="modal-content glass" style="border-radius:24px;">
                <div class="modal-header border-0 pb-0">
